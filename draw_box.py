...
# 全局变量进行路径配置
label_folder = 'data/out/generate_coji_codes/train'  # 检测结果存放文件夹labels路径

raw_images_folder = 'data/out/generate_coji_codes/train'  # 检查图片存放文件夹raw_images路径
...
def make_name_list(raw_images_folder, name_list_path):
    image_file_list = glob.glob(os.path.join(raw_images_folder, '*.jpg'))  # 得到该路径下所有文件名称带后缀
    image_file_list = [t.replace(raw_images_folder, '').replace('\\', '') for t in image_file_list]
